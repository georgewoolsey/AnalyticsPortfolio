...
The r package ['LAGOSNE'](https://cran.r-project.org/web/packages/LAGOSNE/LAGOSNE.pdf) aids users in accessing data from the [Lake Multi-Scaled Geospatial and Temporal Database](https://lagoslakes.org/).

### Loading in data

#### First download and then specifically grab the locus (or site coordinates)
...
```{r, warning=F, message=F, fig.width = 8, fig.height = 5}
## load state polygon file from USAboundaries
states <- us_states()
...
#plot Iowa + Illinois Lakes vs. Minnesota Lakes
...
#plot
ggplot(mn_ia_lakes, aes(lake_area_ha_log, fill = name)) +
...
<span style="color: blue;">This histogram uses the Log10 of lake area due to outliers at the high end of lake area. Below is a  solution to bin the top 5% of lakes based on area into their own bucket in order to display the untransformed lake area on the x-axis. In addition, to get a better comparison between states, we can use the `after_stat(density)` option to plot density on the y-axis instead of count. </span>
